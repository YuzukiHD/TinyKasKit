...
static void set_ahb(void) {
  ...
  write32(T113_CCU_BASE + CCU_PSI_CLK_REG,
          read32(T113_CCU_BASE + CCU_PSI_CLK_REG) | (0x03 << 24));
  sdelay(1);
}

static void set_apb(void) {
  write32(T113_CCU_BASE + CCU_APB0_CLK_REG, (2 << 0) | (1 << 8));
  write32(T113_CCU_BASE + CCU_APB0_CLK_REG,
          (0x03 << 24) | read32(T113_CCU_BASE + CCU_APB0_CLK_REG));
  sdelay(1);
}

static void set_dma(void) {
  /* Dma reset */
  write32(T113_CCU_BASE + CCU_DMA_BGR_REG,
          read32(T113_CCU_BASE + CCU_DMA_BGR_REG) | (1 << 16));
  sdelay(20);
  /* Enable gating clock for dma */
  write32(T113_CCU_BASE + CCU_DMA_BGR_REG,
          read32(T113_CCU_BASE + CCU_DMA_BGR_REG) | (1 << 0));
}
