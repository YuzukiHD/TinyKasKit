...
/*
 * Allwinner D1/T113-DRAM/R528/T113-sx DRAM initialisation
 *
 ...
 * [5] https://github.com/OPI-LINUX/T113-DRAM-melis.git
 */
...
static void sid_read_ldoB_cal(dram_param_t *para) {
  uint32_t reg;
...
static void dram_voltage_set(dram_param_t *para) {
  int vol;
...
static void eye_delay_compensation(dram_param_t *para) // s1
{
  uint32_t delay, i = 0;
  unsigned long ptr;
...
static void mctl_sys_init(dram_param_t *para) {
  ...
  clrbits_le32((T113_CCU_BASE + CCU_DRAM_BGR_REG), (1 << 16));
  clrsetbits_le32((T113_CCU_BASE + CCU_DRAM_CLK_REG), (1 << 31) | (1 << 30),
                  (1 << 27));
  udelay(10);
...
//
static void mctl_com_init(dram_param_t *para) {
  uint32_t val, width;
  unsigned long ptr;
  int i;
 ...
 */
static void mctl_phy_ac_remapping(dram_param_t *para) {
  const uint8_t *cfg;
  uint32_t fuse, val;
  ...
  if (para->dram_type == SUNXI_DRAM_TYPE_DDR2) {
    /* if fuse is 0xa then T113-DRAM -> no remap*/
    if (fuse == 0x0a) {
      sys_uart_printf("T113-DRAM no REMAP!!\r\n");
      return;
    }
    if (fuse == 15)
      return;
...
static int dramc_simple_wr_test(uint32_t mem_mb, int len) {
  ...
  sys_uart_printf("DRAM: simple test OK\r\n");
  return 0;
}

// Set the Vref mode for the controller
//
static void mctl_vrefzq_init(dram_param_t *para) {
...
// settings.
//
static int mctl_core_init(dram_param_t *para) {
  mctl_sys_init(para);
...
static int auto_scan_dram_config(dram_param_t *para) {
  if (((para->dram_tpr13 & (1 << 14)) == 0) &&
    ...
    sys_uart_printf("ERROR: auto scan dram rank & width failed\r\n");
    return 0;
  }

  if (((para->dram_tpr13 & (1 << 0)) == 0) &&
      (auto_scan_dram_size(para) == 0)) {
    sys_uart_printf("ERROR: auto scan dram size failed\r\n");
    return 0;
  }
...
int init_DRAM(int type, dram_param_t *para) {
  ...
  sys_uart_printf("DRAM Type = %d (2:DDR2,3:DDR3)\r\n", para->dram_type);
  if ((para->dram_odt_en & 0x1) == 0)
    sys_uart_printf("DRAMC read ODT off\r\n");
  else
    sys_uart_printf("DRAMC ZQ value: 0x%x\r\n", para->dram_zq);

  /* Test ZQ status */
  if (para->dram_tpr13 & (1 << 16)) {
    sys_uart_printf("DRAM only have internal ZQ\r\n");
  ...
  if ((para->dram_tpr13 & (1 << 0)) == 0) {
    if (auto_scan_dram_config(para) == 0) {
      sys_uart_printf("auto_scan_dram_config() FAILED\r\n");
      return 0;
    }
  }
  ...
  rc = para->dram_mr1;
  if ((rc & 0x44) == 0)
    sys_uart_printf("DRAM ODT off\r\n");
  else
    sys_uart_printf("DRAM ODT value: 0x%x\r\n", rc);

  /* Init core, final run */
  if (mctl_core_init(para) == 0) {
    sys_uart_printf("DRAM initialisation error: 1\r\n");
    return 0;
  }
  ...
  if (para->dram_tpr13 & (1 << 30)) {
    ...
    sys_uart_printf("Enable Auto SR\r\n");
  } else {
...
static struct dram_param param = {
    .dram_clk = 528,
    .dram_type = 2,
    .dram_zq = 0x07b7bf9,
    .dram_odt_en = 0x00,
